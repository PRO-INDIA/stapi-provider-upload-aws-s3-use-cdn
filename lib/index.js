  ...
  init(config) {
    ...
    const getAuth0Token = (Key) => {
      var options = {
        ...
        body: `{"client_id":"${config.auth0ClientId}","client_secret":"${config.auth0ClientSecret}","audience":"${config.auth0audience}","grant_type":"client_credentials"}`,
      };
    ...
    const sendRequestToApi = (apiToken, Key) => {
      console.log(Key);
      const message = { filename: `${Key}` };
      var options = {
        method: "POST",
        url: `${config.apiUrl}blob/sendToQueue/video-transcoding-queue`,
